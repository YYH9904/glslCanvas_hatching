...
void main() {
    ...
    // --- Dramatic far-near effect ---
    // Sharpen mask transition for more obvious separation
    float m = pow(radialMask(uv), 2.5); // Sharper, more dramatic

    // Modulate blur and feedback for far region
    float sigma_far = mix(SIGMA_LP, SIGMA_LP * 2.5, m); // Far region: much blurrier
    float feedback_far = mix(FEEDBACK, FEEDBACK * 1.3, m); // Far region: more feedback
    ...
    if (u_time > 0.02) {
        ...
        float fbAmt = (prevMag > 0.0001) ? feedback_far : 0.0;
        low_accum = mix(ocean_lp, prev, fbAmt);
    }

    // B: 稻 → 高通
    float gain_near = mix(HP_GAIN * 1.7, HP_GAIN, m); // Near region: more detail
    vec3 rice_hp = highPass(u_tex1, uv, SIGMA_HP, gain_near);
